...
import {
  M3_GUIDE_ENTER_DELAY_SECONDS,
  M3_GUIDE_REPEAT_DELAY_SECONDS,
  clampGuideLabelPoint,
  getRelativeGuidePoint,
} from './interactionGuide.js'

const TRACE_SELECTOR = '[data-m3-guide-target="trace"]'
const SATELLITE_SELECTOR = '[data-m3-guide-target="satellite"]'

export default function M3InteractionGuide({ active, visitKey, visualRef }) {
  ...
  useLayoutEffect(() => {
    ...
    media.add(
      ...
      ({ conditions }) => {
        ...
        if (conditions.reduced) {
          ...
          setBeat(staticSelector, '01', '拖动地球或点击目标查看详情')
        ...
        const playLoop = (delay = 0) => {
          timeline = gsap.timeline({
            delay,
            onComplete: () => {
              nextLoop = gsap.delayedCall(M3_GUIDE_REPEAT_DELAY_SECONDS, () => playLoop())
            },
          })
          ...
          timeline
            ...
            .call(() => setBeat(TRACE_SELECTOR, '01', '拖动旋转', { x: -38, y: -14 }))
            .to(cursor, {
              x: destination('x'),
              y: destination('y'),
              autoAlpha: () => targetPoint.current ? 1 : 0,
          ...
          addClickBeat(timeline, TRACE_SELECTOR, '02', '点击坠落记录', 0.48)
          addClickBeat(timeline, SATELLITE_SELECTOR, '03', '点击历史卫星', 0.78)
          timeline.to([cursor, ripple, label], { autoAlpha: 0, duration: 0.28 })
        }

        playLoop(M3_GUIDE_ENTER_DELAY_SECONDS)
    ...
    return () => media.revert()
  }, [active, visitKey, visualRef])

  return (
    <div ref={rootRef} className="m3-interaction-guide" aria-hidden="true">
      <div ref={labelRef} className="m3-interaction-guide-label">
        <span ref={labelIndexRef}>01</span>
        <b ref={labelCopyRef}>拖动旋转</b>
      </div>
      <div ref={cursorRef} className="m3-interaction-guide-cursor">
        <i ref={rippleRef} />
